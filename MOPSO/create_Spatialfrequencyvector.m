...
for i = 1 : nt
    if (i == 1)
    s(1,i) = -1i*(2*pi/lamda)*cos(da)*sin(phi);
    s(2,i) = -1i*(2*pi/lamda)*sin(da)*sin(phi);
    else
         s(1,i) = -1i*(2*pi/lamda)*cos(O(i-1))*sin(phi);
         s(2,i) = -1i*(2*pi/lamda)*sin(O(i-1))*sin(phi);
    end
end
% Matrix of Angle
ma = [da O];
...
save('Data/Matrix_ma','ma')
save('Data/Matrix_s','s')
